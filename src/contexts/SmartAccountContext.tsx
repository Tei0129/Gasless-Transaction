...
export const SmartAccountProvider = ({ children }: any) => {
  ...
  const getSmartAccount = useCallback(async () => {
    ...
    try {
        ...
        networkConfig: [
          {
            ...
            dappAPIKey: process.env.REACT_APP_APIKEY,
          },
        ],
      });
      ...
      state1.address = state.address;
      state1.chainId = state.chainId;
      state1.entryPointAddress = state.entryPointAddress;
      state1.fallbackHandlerAddress = state.fallbackHandlerAddress;
      state1.isDeployed = state.isDeployed;
      state1.owner = state.owner;
      state1.version = state.version;
      state1.implementationAddress = state.implementationAddress;
      console.info("getSmartAccountState", state);
      setState(state1);
      console.info("getSmartAccountState", state1);
